document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateCalendar(month, year) {
        ...
        // Добавляем обработчики событий для каждой даты
...
// Добавьте обработчик событий для каждой ссылки
    links.forEach(link => {
    link.addEventListener('click', (event) => {
        // Предотвратите стандартное действие (открытие в новой вкладке)
        event.preventDefault();

        // Получите URL из атрибута href ссылки
        let url = link.getAttribute('href');

        // Выполните переход на URL
